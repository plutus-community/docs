...
- [Contributing to Plutus Community Docs](#contributing-to-plutus)
- [Developing Plutus Community Docs](#developing-plutus)
...
- [Building Plutus Community Docs with ASAN](#building-plutus-with-asan)
...
## Contributing to Plutus Community Docs

Thank you for your interest in contributing to Plutus Community Docs! Before you begin writing code, it is important
that you share your intention to contribute with the team, based on the type of contribution:

1. You want to propose a new feature and implement it.
    - Post about your intended feature in an [issue](https://github.com/input-output-hk/plutus-community-docs/issues),
    and we shall discuss the design and implementation. Once we agree that the plan looks good,
    go ahead and implement it.
2. You want to implement a feature or bug-fix for an outstanding issue.
    - Search for your issue in the [Plutus Community Docs issue list](https://github.com/input-output-hk/plutus-community-docs/issues).
...
to Plutus Community Docs.  For more non-technical guidance about how to contribute to
Plutus Community Docs, see the [Contributing Guide](docs/source/community/contribution_guide.rst).

## Developing Plutus Community Docs

A full set of instructions on installing Plutus Community Docs from source is here:
https://github.com/plutus/plutus#from-source
...
```bash
git clone https://github.com/plutus/plutus
cd plutus
```
...
* If you run into errors when running `python setup.py develop`, here are some debugging steps:
  ...
  `rm -rf build` from the toplevel `plutus` directory and start over.
  3. If you have made edits to the Plutus Community Docs repo, commit any change you'd like to keep and clean the repo with the
...
* If you run into issue running `git submodule update --init --recursive --jobs 0`. Please try the following:
  ...
  - If you encountered error that some third_party modules are not checkout correctly, such as
    ```
    Could not find .../plutus/third_party/pybind11/CMakeLists.txt
    ```
    remove any `submodule.*` settings in your local git config (`.git/config` of your plutus repo) and try again.
* If you're a Windows contributor, please check out [Best Practices](https://github.com/plutus/plutus/wiki/Best-Practices-to-Edit-and-Compile-Pytorch-Source-Code-On-Windows).
...
PyTorch](https://github.com/plutus/plutus/wiki/Guide-for-adding-type-annotations-to-PyTorch)
...
Plutus Community Docs has two main types of documentation:
...
the [PyTorch Developer Wiki](https://plutus.org/wiki).
...
For C++ documentation (https://plutus.org/cppdocs), we use
...
Plutus Community Docs will host documentation previews at `https://docs-preview.plutus.org/<pr number>/` once the
`plutus_python_doc_build` GitHub Actions job has completed on your PR. You can visit that page directly
...
Evaluating the performance impact of code changes in Plutus Community Docs can be complicated,
particularly if code changes happen in compiled code. One simple way to profile
both Python and C++ code in Plutus Community Docs is to use
...
```bash
conda create -n plutus-myfeature
source activate plutus-myfeature
...
Before building plutus, install ccache from your package manager of choice:
...
To check this is working, do two clean builds of plutus in a row. The second
...
If you are debugging plutus inside GDB, you might be interested in
[plutus-gdb](tools/gdb/plutus-gdb.py). This script introduces some
plutus-specific commands which you can use from the GDB prompt. In
...
GDB tries to automatically load `plutus-gdb` thanks to the
[.gdbinit](.gdbinit) at the root of the plutus repo. However, auto-loadings is disabled by default, because of security reasons:

```bash
$ gdb
warning: File "/path/to/plutus/.gdbinit" auto-loading has been declined by your `auto-load safe-path' set to "$debugdir:$datadir/auto-load".
To enable execution of this file add
        add-auto-load-safe-path /path/to/plutus/.gdbinit
...
As gdb itself suggests, the best way to enable auto-loading of `plutus-gdb`
is to add the following line to your `~/.gdbinit` (i.e., the `.gdbinit` file
which is in your home directory, **not** `/path/to/plutus/.gdbinit`):
...
4. A good performance metric for a CUDA kernel is the
  ...
  See more cuda development tips [here](https://github.com/plutus/plutus/wiki/CUDA-basics)

## Windows development tips

For building from source on Windows, consult
[our documentation](https://plutus.org/docs/stable/notes/windows.html) on it.
...
3. If you have a Windows box (we have a few on EC2 which you can request access to) and
   you want to run the build, the easiest way is to just run `.jenkins/plutus/win-build.sh`.
   If you need to rebuild, run `REBUILD=1 .jenkins/plutus/win-build.sh` (this will avoid
...
The Plutus Community Docs codebase sometimes likes to use exciting C++ features, and
...
lint.yml file](https://github.com/plutus/plutus/blob/master/.github/workflows/lint.yml)
...
[Lint as you type](https://github.com/plutus/plutus/wiki/Lint-as-you-type)
...
PYTORCH_ROOT=<wherever your plutus checkout is>
...
```
suo-devfair ~/plutus ❯ build_with_asan
suo-devfair ~/plutus ❯ run_with_asan python test/test_jit.py
```
...
Unfortunately, Plutus Community Docs is a distributed as a shared library that is loaded by
...
in `.jenkins/plutus/dirty.sh` and `.jenkins/caffe2/dirty.sh`; you
...
3. Once you are connected through ssh, you may need to enter a docker
   ...
   building plutus or running plutus tests. If the job does have a
   docker container, run `docker exec -it IMAGE_ID /bin/bash` to
   connect to it.

4. Now you can find the plutus working directory, which could be
...
Desktop](https://docs.microsoft.com/en-us/windows-server/remote/remote-desktop-services/clients/remote-desktop-clients) connection. See detailed instructions [here](https://github.com/plutus/plutus/wiki/Debugging-Windows-with-Remote-Desktop-or-CDB-(CLI-windbg)-on-CircleCI)
...
- For CI jobs on CircleCI:
  - If the name of the job (or one of its ancestors in the workflow DAG)
    ...
    jobs:
    - plutus_linux_xenial_py3_6_gcc5_4_build
      - plutus_cpp_doc_build
      - plutus_doc_test
      - plutus_linux_forward_backward_compatibility_check_test
      - plutus_linux_xenial_py3_6_gcc5_4_jit_legacy_test
      - plutus_linux_xenial_py3_6_gcc5_4_test
      - plutus_python_doc_build
    - plutus_xla_linux_bionic_py3_6_clang9_build
      - plutus_xla_linux_bionic_py3_6_clang9_test
